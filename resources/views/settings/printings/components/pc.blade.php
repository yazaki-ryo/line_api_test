...
<div class="form-group{{ $errors->{$errorBag ?? 'default'}->has($attribute = 'pc_frame') ? ' has-error' : '' }}">
    ...
    <div class="col-md-7 form-control-static">
        @foreach (array_reverse(\Lang::get('attributes.yes_or_no')) as $key => $item)
            <label>
                <input type="radio" name="{{ $attribute }}" value="{{ $key }}" required {{ (bool)($errors->{$errorBag ?? 'default'}->any() ? old($attribute) : $row->{$attribute} ?? $defaults[$attribute]) === (bool)$key ? 'checked' : ((bool)$key === false ?  'checked' : '') }} /> {{ $item }}
            </label>
        @endforeach
